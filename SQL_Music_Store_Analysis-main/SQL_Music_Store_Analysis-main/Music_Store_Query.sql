...
ORDER by total DESC
LIMIT 10 -- To change Of for ex;- Top 5,3,20
...
--(1) email, first name, last name-- common with a Customer Table])
...
WHERE track_id in(
select track_id from track    --(track id Set for track table , And than)
join genre on track.genre_id = genre.genre_id  --(join for genre table join For genre_id basas)
...
WHERE Milliseconds >(
	  ...
	  from track ) -- OR direct inside this value (393599.212103910933) 
